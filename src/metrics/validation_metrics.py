...
class ValidationTables:
    ...
    def run(self):
        ...
        run_for_every_prediction_path = self.run_for_every_prediction()
        logger.info("Producing tables taking into account the whole time serie")
        run_for_complete_data_path = self.run_for_the_complete_data()

    def run_for_every_prediction(self):
        metrics_train, metrics_test = self.load_metrics_train_test_for_every_run()
    ...
    def run_for_the_complete_data(self):
        datasets_train, datasets_test = self.load_data_train_test_for_entire_set()
        ...
        dict_to_use_to_save = {"train": data_train_agg, "test": data_test_agg}
        csv_paths = []
        for key, value in dict_to_use_to_save.items():
            city = "".join(self.location.city.split(" ")).lower()
            country = "".join(self.location.country.split(" ")).lower()
    ...
    def load_metrics_train_test_for_every_run(self) -> tuple:
        metrics_train = []
        metrics_test = []
    ...
    def load_data_train_test_for_entire_set(self) -> tuple:
        data_train = []
        data_test = []
        for dataset in self.validation_datasets:
