...
class ScoresTables:
    ...
    def __init__(
        ...
        self.observations = xr.open_dataset(
            station_path_observations, chunks={"time": 100}
        )
        self.forecasts = xr.open_dataset(station_path_forecast, chunks={"time": 100})
        params = station_path_observations.name.split("_")
        self.location_parameters = {}
        for i, param in enumerate(
            ["variable", "country", "city", "location_id", "time_range"]
        ):
    ...
    def run(self):
        ...
        tables = [
            self.nmae_tables(diff, self.observations),
        ...
        table_merged.to_excel(output_path, float_format="%.2f")
    ...
    def pearson_tables(self, forecast: xr.Dataset, observation: xr.Dataset):
        varname = self.location_parameters["variable"]
        pearson_correlation = weighted_corr(
            forecast[varname],
            observation[varname],
            dim="time",
            weights=None,
    ...
    def merging_tables(self, tables):
        table_merged = pd.concat(
            tables,
            axis=1,
            keys=("NMAE", "Bias", "RMSE", "De-Biased NMAE", "Pearson Correlation"),
        )
        return table_merged
