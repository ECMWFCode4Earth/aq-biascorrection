...
class CAMSProcessor:
    ...
    def get_total_data(
        self, initialization_times: List[str]
        ...
        total_data = xr.open_mfdataset(intermediary_paths, concat_dim="time")
        return total_data, intermediary_paths

    def get_data_for_initialization_time(self, initialization_time: str) -> Path:
        """
        Get the data for one initialization_time of all the different locations
        of interest given in the .csv file
        """
        logger.info(f"Getting data for initialization time" f" {initialization_time}")
        try:
    ...
    def get_paths_for_forecasted_variables(
        ...
        if len(paths) == 0:
            raise Exception(
                f"There is no data for the initialization time:"
