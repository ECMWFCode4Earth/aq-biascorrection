...
class OpenAQDownloader:
    ...
    def __init__(
        ...
        if variable in ["o3", "no2", "so2", "pm10", "pm25"]:
            ...
            raise NotImplementedError(
                f"The variable {variable} do" f" not correspond to any known one"
            )
    ...
    def run(self) -> Path:
        ...
        if not output_path_data.exists():
            ...
            logger.info(
                f"Data has been correctly downloaded" f" in {str(output_path_data)}"
            )
        else:
    ...
    def get_closest_stations_to_location(self) -> pd.DataFrame:
        ...
        for station in stations.iterrows():
            ...
            if pd.to_datetime(self.time_range["start"]) >= station[
                "firstUpdated"
            ].tz_convert(None) and pd.to_datetime(self.time_range["end"]) <= station[
                "lastUpdated"
            ].tz_convert(
                None
            ):
                is_in_temporal_range.append(1)
            elif pd.to_datetime(self.time_range["end"]) < station[
                "firstUpdated"
            ].tz_convert(None):
                is_in_temporal_range.append(-1)
            else:
        ...
        stations["combination_dist_and_is_in_temporal_range"] = (
    ...
    def _get_closest_stations_to_location(self) -> pd.DataFrame:
        ...
        stations = self.api.locations(
            ...
            coordinates=f"{self.location.latitude}," f"{self.location.longitude}",
            radius=100000,
            df=True,
        )
        ...
        if len(stations) == 0:
            ...
            raise Exception(
                "There are no stations next to this location in "
    ...
    def check_variable_in_station(self, station: pd.Series):
        ...
        if self.variable not in [x["parameter"] for x in station["parameters"]]:
            raise Exception(
                "The variable intended to extraction is not"
                " available for the nearest / exact location"
            )

    @staticmethod
    def concat_filter_and_save_data(datasets: List[xr.Dataset], output_path_data: Path):
        """
    ...
    def create_xarray_dataset_with_attrs(
        self, data: pd.DataFrame, station: pd.Series
    ) -> xr.Dataset:
        xr_ds = (
            data["value"]
            .to_xarray()
            .rename({"date.utc": "time"})
            .to_dataset(name=self.variable)
        )
        ...
        if len(units) == 1:
            if isinstance(units[0], bytes):
                units[0] = units[0].decode("utf-8")
            ...
            logger.error(
                f"Units of {self.variable} observations downloaded "
